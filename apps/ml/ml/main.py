...
def load_model():
    ...
    model = resnet34(weights=ResNet34_Weights.IMAGENET1K_V1)  # Use the correct enum for weights
    num_classes = 10
...
async def predict(file: UploadFile = File(...)):
    ...
    try:
        ...
        logger.info(f"Prediction made: {prediction} - {category_map[prediction]} with confidence {outputs.softmax(1)[0][prediction].item():.4f}")
